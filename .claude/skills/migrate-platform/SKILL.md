...
| Factorial client | custom `factorial_client`/`auth_factorial` → **`FactorialClient`** from `base-app`; credentials/env auth → provisioned `FACTORIAL_TOKEN` (OAuth scopes must be declared at App creation — list the scopes the code needs) |
| Deps | `# @add-package` works as-is; also register in `dependencies/requirements.txt` |
| Deploy | `scripts/deploy.py` + GitHub Action → `fcode` CLI flow (`clone dev-{id}`, `push`, release request); decide the repo's new sync model |
